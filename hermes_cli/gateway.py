...
def _get_windows_gateway_service_records() -> list[dict[str, object]]:
    """Return running Windows services that appear to manage Hermes gateway."""
    if not _IS_WINDOWS:
        return []
    ...
    for row in rows:
        ...
        records.append({
            ...
            "path": str(row.get("PathName", "")).strip(),
        })
    return records


def _find_windows_services_for_gateway_pids(pids: list[int]) -> list[dict[str, object]]:
    """Return Windows service records that are ancestors of gateway PIDs."""
    if not _IS_WINDOWS or not pids:
        return []

    service_by_pid = {
        int(record["process_id"]): record
        for record in _get_windows_gateway_service_records()
        if isinstance(record.get("process_id"), int)
    }
    if not service_by_pid:
        return []
    ...
    for pid in pids:
        ...
        for _ in range(12):
            ...
            if parent is None or parent in seen:
                break
            seen.add(parent)
            service = service_by_pid.get(parent)
            if service:
                name = str(service.get("name", "")).strip()
                if name:
                    matches[name] = service
                break
            current = parent
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                windows_services = _find_windows_services_for_gateway_pids(pids)
                print(f"✓ Gateway is running (PID: {', '.join(map(str, pids))})")
                if windows_services:
                    service_names = ", ".join(
                        str(service.get("display_name") or service.get("name") or "").strip()
                        for service in windows_services
                    )
                    print(f"  (Running as a Windows service: {service_names})")
                else:
